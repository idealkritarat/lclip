...
`lanclipd` is the only process that opens an Iroh endpoint. `lcp` is a thin client that talks to it over local IPC (a Unix domain socket on macOS, a named pipe on Windows) and never holds network state of its own. See ADR [[0005-daemon-owns-network-state]] and [[0003-iroh-instead-of-lan-tcp]] for why.
...
- accept incoming connections and store messages in RAM even when no terminal is open,
...
The CLI only sends requests over IPC, receives responses/events, and reads/writes the local system clipboard in its own foreground user session.

## Transport decision

Iroh is responsible for endpoint identity, QUIC transport, encryption, NAT traversal, direct-path selection, relay fallback, and address lookup. The application is responsible for pairing workflow, the trusted-peer allowlist, message schema/validation, ACK/dedup, conversation state, the CLI, and local IPC. See `docs/protocol.md` for the wire-level detail and `docs/security.md` for the trust model.
